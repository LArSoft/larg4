...
namespace evgen{
  ...
  template <typename OptionList>
  auto larg4SingleGen::selectOption
    (std::string Option, OptionList const& allowedOptions) -> decltype(auto)
  {
  ...
  template <typename OptionList>
  std::string larg4SingleGen::optionName(
    typename OptionList::value_type::first_type optionKey,
    OptionList const& allowedOptions,
    std::string defName /* = "<unknown>" */
  ) {
  ...
  void larg4SingleGen::setup()
  {
    ...
    if(list.size() > 0)
      throw cet::exception("larg4SingleGen") << "The "<< list
...
//    else {std::cout << "do not understand the value of PDist!";}
  ...
  void larg4SingleGen::Sample(simb::MCTruth &mct)
  {
	...
	SampleOne(i,mct);
      ...
      break;
    default:
      mf::LogWarning("UnrecognizeOption") << "larg4SingleGen does not recognize ParticleSelectionMode "
					  << fMode;
      break;
    } // switch on fMode
  ...
  void larg4SingleGen::printVecs(std::vector<std::string> const& list)
  {
    ...
    mf::LogInfo("larg4SingleGen") << " You are using vector values for larg4SingleGen configuration.\n   "
  ...
  //____________________________________________________________________________
  void larg4SingleGen::SelectFromHist(const TH2& h, double &x, double &y) // select from a 2D histogram
  {
    CLHEP::RandFlat   flat(fEngine);
